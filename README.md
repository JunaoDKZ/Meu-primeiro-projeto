# Meu-primeiro-projeto
![layout-finalizado](https://github.com/user-attachments/assets/ccc7d084-af1c-49af-9689-b5f34de13189)
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Parque Estadual Ilha Bela</title>

</head>
<body>
    <div>
        <img src="parque-ilha-bela.jpg" alt="projeto01"
        title="Projeto">

<h1>O lugar ideal para momentos inesquecíveis</h1>
<p>Fundado em 1992, o Parque Estadual Ilha Bela é o lugar ideal para você 
desfrute de ótimos momentos. Anualmente 2 milhões de visitantes passam pelo 
parque para conhecer o local. Veja o que o parque oferece:  </p>
    </div>
<div>
<h2> Tranquilidade e segurança </h2>
<p> Por todo o parque é possível ver um clima de tranquilidade e segurança. 
É o lugar perfeito para se reunir com amigos, familiares ou até mesmo para 
visitar sozinho. </p>
</div>
<div>
<h3> Paisagens de tirar o fôlego  </h3>
<p> Diversos tipos de paisagens podem ser vistas ao longo do parque, 
    então é garantia de fotos incríveis de todos os tipos. </p>
</div>
<div> 
<h4>Entrada Franca </h4>
<p>A entrada no parque é totalmente gratuita. A exceção fica por conta de 
    algumas atividades que podem ser feitas no local. Para mais informações 
    entre em contato clique aqui.  </p>
    Para mais informações 
    entre em contato <a href="tel:+5521999999999">Clique aqui</a>

</div>

</body>
</html>
